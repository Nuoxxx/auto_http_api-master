    ...
    def function(self):
        ...
        if self.index == 1: #第一个xml预留的主要是登陆
             if gm[self.index]["method"] == "POST":
                go.REALLY_RESULT = self.config_http.post(url=gm[self.index]["url"],
                                                         params=hp.str__post_param(gm[self.index]["param"]))
...
def run_case(runner):
    ...
    if len(case_list) == False: #判断是否执行指定的用例ID
        ...
        for index in range(1, len(temp_case)):
            ...
            # 记录运行结果
            mresult.info.append(json.loads(json.dumps(resultInfo(MresultDetail.resultInfo(), t_id=gm[index]["id"],
                                                                 t_name=gm[index]["name"],
        ...
        for i in case_list:
            for j in range(1, len(gm)):
                if str(i) == gm[j]['id']:
                    ...
                    runner.run(test_suite)
                    # 记录运行结果
                    mresult.info.append(json.loads(json.dumps(resultInfo(MresultDetail.resultInfo(),
                                                                         t_id=gm[j]["id"], t_name=gm[j]["name"],
...
if __name__ == '__main__':
    ...
    data = json.loads(json.dumps(result(mresult, test_date=str(sum_time) + "毫秒",
                                        test_sum=go.CASE_TOTAL, test_failed=go.ERROR_NUM,
                                        test_version="v1.0", test_pl="python2.7",
